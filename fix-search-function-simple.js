...
async function fixSearchFunction() {
  ...
  try {
    console.log('Creating simple search function...');
    
    // First, let's add project_id column to video_moments if it doesn't exist
    try {
      await client.query('ALTER TABLE video_moments ADD COLUMN project_id UUID REFERENCES projects(id)');
      console.log('Added project_id column to video_moments');
    } catch (error) {
